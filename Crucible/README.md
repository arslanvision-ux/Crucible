...
*(Note: Windows users can simply run `install_nuke_windows.bat`, and macOS/Linux users can run `install_nuke_unix.sh` for an automated installation).*

### Houdini, Maya, Blender
Please see the `install/README.md` file inside the Crucible folder for instructions on installing the Live Bridge for your specific DCC.
